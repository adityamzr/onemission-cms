    ...
    <main class="grow" role="content">
        ...
        <div class="container-fixed flex items-center justify-between flex-wrap gap-3">
            ...
            <div class="flex items-center flex-wrap gap-1.5 lg:gap-3.5">
                ...
                <button type="button" wire:click="$dispatchTo('discounts.discount-form', 'openModal')" class="btn btn-dark">
                        ...
                        <div id="kt_remote_table">
                            <div class="scrollable-x-auto">
                                <table class="table table-auto table-border align-middle text-gray-700 font-medium text-sm" data-datatable-table="true">
                                    ...
                                    <tbody>
                                        @if (empty($discounts))
                                            <span class="text-secondary-foreground text-sm"> Empty Data </span>
                                        @endif
                                        @foreach ($discounts as $key => $item)
                                        <tr>
                                            <td class="text-center">{{ $discounts->firstItem() + $key }}</td>
                                            <td>{{ $item->code }}</td>
                                            <td>{{ $item->type }}</td>
                                            ...
                                            @endif
                                            @if ($item->min_purchase == null)
                                                <td> - </td>
                                            @else
                                                <td>{{ formatRupiah($item->min_purchase) }}</td>
                                            @endif
                                            <td>{{ $item->expires_at }}</td>
                                            <td class="flex items-center gap-2">
                                                <button type="button"  wire:click="$dispatchTo('discounts.discount-form', 'openModal', { id: {{ $item->id }} })" class="btn btn-warning btn-sm">
                                                    <i class="ki-filled ki-pencil">
                                                    </i>
                                                    Edit
                                                </button>
                                                <button type="button" wire:confirm="Are you sure you want to delete this data?" wire:click="destroy({{ $item->id }})" class="btn btn-danger btn-sm">
                            ...
                            <div class="card-footer justify-between flex-col md:flex-row gap-3 text-gray-600 text-2sm font-medium">
                                ...
                                {{ $discounts->links('vendor.livewire.tailwind') }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- End of Container -->
        @livewire('discounts.discount-form')
    </main>
</div>
